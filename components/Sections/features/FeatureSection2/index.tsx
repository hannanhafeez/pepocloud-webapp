...
export type FeatureSection2Props = {
	FeatureSection2Data? : Array<{
		img: string, heading?: string, description?: string
	}>
}

const FeatureSection2: FC<FeatureSection2Props> = ({ }) => {

	const { data, error } = useSWR(FEATURES.section2.heading, fetcher);
	const { data: cardList, error: cardsError } = useSWR(FEATURES.section2.cards, fetcher);
			...
			<div className='container mx-auto py-10 md:py-[7.5rem] lg:py-20 gap-x-8 flex flex-col lg:flex-row items-stretch gap-8 sm:gap-10'>
				...
				<div className='flex-[1]'>
					<h2 className={commonCss.section_heading}>
						{isHeadingReady && (data?.[0].title || 'Appointment Scheduling made Easy')}
					</h2>
				</div>
